...
def main():
    ...
    try:
        ...
        for num in range(len(userid)):
            ...
            notice = '一卡通号' + userid[num] + '已于' + time.strftime('%m月%d日%H：%M完成当日健康打卡\n')
            print(notice)
            with open(file='log.txt', mode='a') as file:
                file.write(notice)

    except Exception as r:
        print('Error:', r, error_type(step, userid[num]))
        with open(file='log.txt', mode='a') as file:
            file.write('Error!' + r.__str__() + error_type(step, userid[num]) + time.strftime(' %m-%d %H:%M\n'))
